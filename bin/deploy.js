...
aws.command(`s3api get-bucket-location --bucket "${config.bucket}"`).then().catch(() => {
  ...
  return aws.command(`s3 website "s3://${config.bucket}" --index-document index.html --region ${config.region}`);
}).then(() => {
  // Set bucket policy
  ...
  return aws.command(`s3 sync ./html/ "s3://${config.bucket}" --region ${config.region} --content-type "text/plain" --exclude "*.*"`);
}).then(() => {
  // Sync the rest, deleting any unused files
  return aws.command(`s3 sync ./html/ "s3://${config.bucket}" --region ${config.region} --delete`);
}).then(() => {
  console.log(`http://${config.bucket}.s3.${config.region}.amazonaws.com`)
}).catch(error);
